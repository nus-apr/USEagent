...
from useagent.tools.edit import __reset_project_dir, create, init_edit_tools
...
@pytest.mark.tool
def test_reset_after_initialization_should_be_zero():
    init_edit_tools(".")
    assert edit._project_dir == Path(".")
    __reset_project_dir()
    assert not edit._project_dir
...
@pytest.mark.regression
@pytest.mark.tool
async def test_using_tool_without_initialization_throws_assertion_error_explicit_reset(
    tmp_path,
):
    __reset_project_dir()
    file = tmp_path / "new_file.txt"
