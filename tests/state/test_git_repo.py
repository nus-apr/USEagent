...
def test_init_for_existing_git_repo(tmp_path):
    ...
    subprocess.run(
        ["git", "commit", "-m", "Pre-existing commit"], cwd=tmp_path, check=True
    )
...
def test_git_diff_additions_after_initialization_git_diff_will_show_files(tmp_path):
    ...
    subprocess.check_output(["git", "diff"], cwd=tmp_path).decode()
...
def test_gitignore_file_is_not_committed(tmp_path: Path):
    ...
    subprocess.check_output(["git", "log", "--oneline"], cwd=tmp_path).decode()
