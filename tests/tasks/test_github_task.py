...
def test_clone_into_directory(tmp_path):
    ...
    GithubTask("Issue", f"file://{repo_path}", clone_path)
...
def test_overwrites_existing_dir(tmp_path):
    ...
    GithubTask("Issue", f"file://{repo_path}", target_dir)
...
def test_checkout_commit_and_create_branch(tmp_path):
    ...
    GithubTask("Issue", f"file://{repo_path}", clone_path, commit=sha)

    current_branch = (
        subprocess.check_output(["git", "branch", "--show-current"], cwd=clone_path)
        .decode()
        .strip()
    )
    current_commit = (
        subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=clone_path)
        .decode()
        .strip()
    )
...
def test_checkout_known_octocat_commit(tmp_path, commit):
    ...
    GithubTask("Issue", repo_url, dest, commit=commit)

    current_commit = (
        subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=dest).decode().strip()
    )
